...
当一名我方单位被选中并按 `OK` 进入行动时，采用**两级交互**：先在动作菜单中选动作类型，再在地图上拾取目标格。

1. **动作菜单**：进入行动阶段时，半透明面板列出当前可用动作类型——「移动」（存在可达格时）、「攻击」（存在可攻击目标时）、「待机」（恒有，主动结束该单位行动的唯一途径）。`UP` 选中上一项、`DOWN` 选中下一项，首尾循环；`OK` 确认；提示栏显示剩余 AP；
2. **地图拾取**：菜单确认「移动」或「攻击」后面板隐藏，进入拾取态。该类型全部候选格构成有序列表：按方位环固定顺序（上、右、下、左），同方位内按到单位的距离升序，同距离再按 y、x。当前候选格以高亮边框显示，其余候选格以淡化 1px 边框显示（移动蓝、攻击橙）；`UP`/`DOWN` 首尾循环（自最后一项继续按 `DOWN` 回到第一项）；`OK` 执行当前候选；
3. **返回菜单**：执行动作后，若该单位仍有可用动作（剩余 AP 或尚未攻击且存在目标）则重新回到动作菜单；若候选仅剩「待机」或为空，自动待机并切换到下一个单位（选人状态）；
4. 排序必须稳定：**同一状态下反复按 `UP`/`DOWN` 必须产生完全可预测的顺序**，不得使用任何影响顺序的随机因素。

H2 验证结论（2026-09-24 实机试玩）：原「距离优先」顺序在连续按 `DOWN` 时方向感破碎——高亮在四个方位间来回跳，体感如同随机走位。已按本节预案调整为方位环优先：连续按 `DOWN` 沿一个方位由近及远推进，跨方位才进入下一环。配套新增候选范围淡色边框显示（见第 2 条）。

二次试玩结论（同日）：全部候选（移动格 + 攻击目标 + 待机）混排于单列表时，动作类型切换不显眼且攻击项埋在大量移动格之后。交互重构为本节现行「动作菜单 + 地图拾取」两级方案；单一候选循环（历史方案）保留为 P1 备选。
...
| 战场 · 我方回合 · 动作菜单 | 上一动作类型 | 下一动作类型 | 进入对应拾取态 / 执行「待机」 | 暂停菜单 |
| 战场 · 我方回合 · 地图拾取 | 上个候选格 | 下个候选格 | 执行高亮候选 | 暂停菜单 |
